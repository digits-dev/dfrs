<?php namespace App\Http\Controllers;
	...
	class AdminCustomersController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->limit = "20";
			$this->orderby = "customer_code,asc";
			...
			$this->form = [];
			$this->form[] = ['label'=>'Customer Code','name'=>'customer_code','type'=>'text','validation'=>'required|min:1|max:20','width'=>'col-sm-6'];
			$this->form[] = ['label'=>'Customer Name','name'=>'customer_name','type'=>'text','validation'=>'required|min:1|max:150','width'=>'col-sm-6'];
	        ...
	        */
            ...
            if(CRUDBooster::getCurrentMethod() == 'getIndex'){
                $this->index_button[] = ['label'=>'Import '.CRUDBooster::getCurrentModule()->name,'url'=>route('customer.upload-view'),'icon'=>'fa fa-upload','color'=>'warning'];
            }
	    ...
	    */
        ...
        public function customerUploadView()
        {
            ...
            $data = [];
            $data['page_title'] = 'Upload Customers';
            $data['uploadRoute'] = route('customer.upload');
            $data['uploadTemplate'] = route('customer.template');
        ...
        public function customerUpload(Request $request)
        {
            ...
            //check headings
            $header = config('excel-template-headers.customer');
			...
			if(!empty($unMatch)) {
                return redirect(route('customer.upload-view'))->with(['message_type' => 'danger', 'message' => 'Failed ! Please check template headers, mismatched detected.']);
			}
            HeadingRowFormatter::default('slug');
            // $excelData = Excel::toArray(new CustomerImport, $path);
        ...
        public function uploadTemplate()
        {
            ...
            return Excel::download($export, 'customer-'.date("Ymd").'-'.date("h.i.sa").'.csv');
        }
