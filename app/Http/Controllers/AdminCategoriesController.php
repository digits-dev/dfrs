<?php namespace App\Http\Controllers;
	...
	class AdminCategoriesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form = [];
			$this->form[] = ['label'=>'Category Code','name'=>'category_code','type'=>'text','validation'=>'required|min:1|max:20','width'=>'col-sm-6'];
			$this->form[] = ['label'=>'Category Name','name'=>'category_name','type'=>'text','validation'=>'required|min:1|max:150','width'=>'col-sm-6'];
	        ...
	        */
            ...
            if(CRUDBooster::getCurrentMethod() == 'getIndex'){
                $this->index_button[] = ['label'=>'Import '.CRUDBooster::getCurrentModule()->name,'url'=>route('category.upload-view'),'icon'=>'fa fa-upload','color'=>'warning'];
            }
	    ...
	    */
        ...
        public function categoryUploadView()
        {
            ...
            $data = [];
            $data['page_title'] = 'Upload Categories';
            $data['uploadRoute'] = route('category.upload');
            $data['uploadTemplate'] = route('category.template');
            return view('category.upload',$data);
        }
        ...
        public function categoryUpload(Request $request)
        {
            ...
            //check headings
            $header = config('excel-template-headers.category');
			...
			if(!empty($unMatch)) {
                return redirect(route('category.upload-view'))->with(['message_type' => 'danger', 'message' => 'Failed ! Please check template headers, mismatched detected.']);
			}
            HeadingRowFormatter::default('slug');
            // $excelData = Excel::toArray(new CategoryImport, $path);
        ...
        public function uploadTemplate()
        {
            ...
            return Excel::download($export, 'category-'.date("Ymd").'-'.date("h.i.sa").'.csv');
        }
