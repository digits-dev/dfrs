...
<div class='panel panel-default'>
    ...
    <div class='panel-body'>
        ...
        <div class="col-md-12">
            ...
            <table class="table table-bordered" id="pnl-company-total">
                <thead style="background-color: #0047ab; color:white;">
                    <tr>
                        <th class="text-center" width="30%">ALL</th>
                    ...
                    @foreach (config('pnl-template.pnl_1') as $key_pnl => $pnl_header)
                        ...
                        <tr>
                            ...
                            @foreach($columnYear as $keyYear => $year)
                                ...
                                @if ($pnlHeader == 'revenue')
                                    <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}">{{ number_format($revenues_sum[0]->$year,2) }}</td>
                                    <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">100%</span></th>
                                @elseif ($pnlHeader == 'cost-of-sales')
                                    <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}">{{ number_format($cogs_sum[0]->$year,2) }}</td>
                                    <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">100%</span></th>
                                @else
                        ...
                        @else
                            ...
                            <tr>
                                ...
                                @foreach($columnYear as $keyYear => $year)
                                    @if ($key_pnl == 'OPERATING EXPENSES')
                                        @foreach ($opex_pivot_sum as $key_opx => $opx )
                                            @if ($opx->chart_account_subtype == $row_pnl)
                                                <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}" data-name="{{ $row_pnl }}">{{ number_format($opx->$year,2) }}</td>
                                                <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">0%</span></th>
        ...
        <div class="col-md-12">
            ...
            <table class="table table-bordered" id="pnl-company{{ $key_company }}">
                <thead style="background-color: #0047ab; color:white;">
                    <tr>
                    ...
                    @foreach (config('pnl-template.pnl_1') as $key_pnl => $pnl_header)
                        ...
                        @if (!is_array($pnl_header))
                            ...
                            <tr>
                                ...
                                @foreach($columnYear as $keyYear => $year)
                                    @if ($pnlHeader == 'revenue')
                                        <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}">{{ number_format($revenues_data[$key_company]->$year,2) }}</td>
                                        <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">100%</span></th>
                                    @elseif ($pnlHeader == str_replace(' ', '-', strtolower($cogs_data[$key_company]->chart_account_subtype)) && $cogs_data[$key_company]->inter_company_name == $company)
                                        <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}">{{ number_format($cogs_data[$key_company]->$year,2) }}</td>
                                        <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">0%</span></th>
                                    @else
                                        <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}">0.00</td>
                                        <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">0%</span></th>
                        ...
                        @else
                            ...
                            @foreach($pnl_header as $row_key => $row_pnl)
                                <tr>
                                    <td>
                                        {{ $row_pnl }}
                                    </td>
                                    ...
                                    @foreach($columnYear as $keyYear => $year)
                                        ...
                                        @if ($key_pnl == 'OPERATING EXPENSES')
                                            @foreach ($opex_data as $key_opx => $opx )
                                                @if ($opx->chart_account_subtype == $row_pnl && $opx->inter_company_name == $company)
                                                    <td class="{{ $pnlHeader }}-amount" data-id="{{ $year }}" data-name="{{ $row_pnl }}">{{ number_format($opx->$year,2) }}</td>
                                                    <th><span class="{{ $pnlHeader }}-percentage-{{ $year }}">0%</span></th>
...
<script type="text/javascript">
    ...
    document.title="PNL Report By Company";
    ...
    var pnlTable = document.getElementById('pnl-company-total');
    if(pnlTable) {
